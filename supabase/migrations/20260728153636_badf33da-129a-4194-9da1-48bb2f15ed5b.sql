...
BEGIN
  ...
  SELECT cgpa, standing, auth_user_id INTO v_cgpa, v_cur_std, v_auth_user
  ...
  IF v_new_std IN ('probation','withdrawn') AND v_new_std IS DISTINCT FROM v_cur_std AND v_auth_user IS NOT NULL THEN
    ...
    VALUES (v_auth_user, v_notify_title, v_notify_body, 'academic_standing');
  END IF;
END; $function$;