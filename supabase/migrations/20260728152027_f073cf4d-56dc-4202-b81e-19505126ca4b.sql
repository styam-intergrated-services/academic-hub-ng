...
GRANT ALL ON public.import_payloads TO service_role;

ALTER TABLE public.import_payloads ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "import_payloads_service_only" ON public.import_payloads;
CREATE POLICY "import_payloads_service_only" ON public.import_payloads
...
BEGIN
  ...
  FOR r_stu IN SELECT value FROM jsonb_array_elements(_payload->'students') LOOP
    ...
    END IF;
    v_student_by_matric := v_student_by_matric || jsonb_build_object(v_matric, v_student_id::text);
